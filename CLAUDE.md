...
## Current State (Run 47 — Stun Status + Concussive Slam Ability)
### Implemented ✅
...
3. **Vulnerability status effect** — Run 47 added stun (the 8th status,
   paired with the new Concussive Slam unlock for Brawler), but the
   roadmap's "status-effect depth" item still calls out vulnerability —
   a debuff that AMPLIFIES incoming damage (e.g., target takes +50% from
   the next hit). Pairs naturally with the Arcanist's burst pattern
   (frost_nova → vulnerable → fireball combo). Engine integration is one
   line in `_calculate_damage` (a `damage_taken_mod` field on the status,
   read alongside the existing `damage_multiplier` from vanished).

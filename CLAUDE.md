...
- **`stack()` collapser** — regenerating SUMS `heal_per_turn` (mirrors the existing `damage_per_turn` summer). Two `+6/turn` applications stack to `12/turn` for the longer of the two durations (via the existing `duration` MAX). Matches poison_blade's DoT compounding so the player's "stacking the buff is good" expectation holds consistently across positive and negative tickers.
- **`src/combat/Combatant.gd`** — `tick_statuses()` now reads `heal_per_turn` after the existing `damage_per_turn` drain and calls `heal()` on the carrier. Order matters: DoT first so a carrier on the brink can still die to burning even if a regen tick would have papered it over — that keeps the "your HoT didn't save you" beat consistent with how poison-then-heal plays in every other tactical roguelike. `heal()` clamps to `max_hp - hp`, so a full-HP carrier wastes the tick (no overheal pool). Skip-gate is `is_alive()` — a carrier killed by the DoT tick the same turn does NOT get revived by the heal. Return contract stays as total damage taken (NOT net heal) so existing callers (BattleScene damage numbers, boss-phase triggers) don't have to know about heals.
- **`src/data/Abilities.gd`** — new `iron_resolve` ability. Schema: `type: buff`, `target: self`, `range: 0`, `base_damage: 0` (it's a pure self-buff), `max_charges: 2`, `cooldown_turns: 4`, `xp_cost: 50`, `icon_key: shield`, `applies_regenerating: true`, `regen_duration: 3`, `regen_hpt: 8`. Default 8 hpt × 3 turns = 24 total HP — meaningful sustain without trivializing late-game floor damage. 2 charges / 4-turn cooldown matches mana_shield's "defensive self-buff" tempo so the cross-class self-buff pool reads consistently. Distinct from the existing `heal_big` LevelUp upgrade (instant 50 HP, single use, no per-turn pacing).
- **`scenes/LevelUp.gd`** — `CLASS_UNLOCKS["brawler"]` now reads `["concussive_slam", "iron_resolve", "shield_bash"]`. The Run-47 flagship `concussive_slam` stays at index 0 (the class-unique stun), `iron_resolve` slots in at index 1 (the second class-unique pick, sustain flavor), and `shield_bash` stays as the older fallback. The Brawler now has TWO class-unique picks (tempo + sustain), closing the "only damage cards" gap their unlock list had through Run 47.
- **`scenes/BattleScene.gd`** — new `iron_resolve` match arm in `_do_hero_self_ability`, sitting alongside the existing `taunt` / `vanish` / `mana_shield` arms. Reads `regen_duration` + `regen_hpt` from the ability data, calls `StatusEffect.regenerating(...)`, applies via `_hero.apply_status`, refreshes the status label, plays the taunt VFX (warm shield glow — the defensive-self-buff visual reads the same beat), flashes the hero's hex in green (`Color(0.30, 0.92, 0.42)` — matches the `hp_up` upgrade color from LevelUp), and emits a combat-log line `Carl -> IRON RESOLVE +8/turn x3` in the same green. Effect-texture map gains `iron_resolve -> fx_taunt.png`. The REG short-code above the sprite carries the lasting "this hero is healing each turn" advertisement for the buff's 3-turn window.
- **`autoloads/SystemVoice.gd`** — new `iron_resolve` quip pool (6 lines, DCC-flavored: "Iron Resolve. The hero's body has agreed to mend itself, briefly.").
- **`tests/test_run49.gd`** (28 test functions, ~76 assertions): factory shape (default 3-turn / +6 hpt, no DPT, no armor mod), custom hpt carries (12/turn), negative-input clamps (negative duration AND negative hpt both go to 0 — clamping the hpt protects against a corrupted save secretly draining HP), zero-hpt allowed as a no-op tracker; HUD helpers — short_code returns "REG", display_name returns "Regenerating", summarize includes "+N HP/turn" + carries duration, summarize omits "armor"/"skip turn"/"taken" (no spurious armor mod, skip-turn, or damage-taken segments), custom hpt formatting (+12 HP/turn not +1), 0-hpt omits the heal-rate segment; stack collapses two regenerates with SUMMED hpt (6 + 4 = 10) and MAX duration, MAX direction-independent (shorter first, longer second still picks longer + still sums). Iron Resolve schema — in `Abilities.DATA`, has all required fields, in `LevelUp.CLASS_UNLOCKS["brawler"]` between concussive_slam (Run 47) and shield_bash, 2 charges / 4-turn cooldown / 50 xp tuning locked, default hpt = 8 and duration = 3t. Combatant.tick_statuses — heal lands on wounded carrier (30 → 36 with default 6/turn), no overheal (full-HP carrier stays at max), partial heal at max boundary (97 + 6 clamps to 100), 3-turn buff expires after exactly three ticks and totals 18 (3 × 6), DoT-then-revive regression guard (a carrier killed by burning the same tick is NOT revived by the regen — the `is_alive()` gate skips the heal), DoT-first heal-second order (burn drains before regen heals; return value is the burn damage, not net), 0-hpt is a silent no-op, two regen stacks heal combined (6 + 4 = 10 net per tick). End-to-end Iron Resolve payload: a 40/100 Brawler heals 24 over 3 turns (40 → 64), regenerating expires cleanly, mana_shield untouched by regen apply (regression guard against a defensive caller routing the buff through take_damage). Cross-effect isolation: regenerating doesn't carry skips_turn, doesn't register as frozen/stunned, doesn't carry damage_taken_mod (a heal-over-time that secretly amped incoming damage would be a debuff in disguise).
- **Test suite total: 3022 passed, 0 failed** (up from 2946 baseline; +76 new).

**Run 48 (Vulnerable status effect + Arcane Sunder ability — roadmap item #3 from the Run-47 audit; closes the "status-effect depth" loop the roadmap has tracked since Run 35 and gives the Arcanist a class-unique unlock that primes their burst pattern):**
- **`src/combat/StatusEffect.gd`** — new `vulnerable(duration: int = 2, amp_pct: int = 50) -> Dictionary` factory. The 9th status type (after Run 47's stun) and the first DAMAGE-AMP debuff — stores `damage_taken_mod = 1.0 + amp_pct/100.0` (a float multiplier on raw damage) plus the integer `damage_taken_pct` for HUD use. Distinct from every previous debuff: doesn't deal DPT (unlike burning/poisoned/bleed), doesn't change armor (unlike frozen/fortified), doesn't lock a turn (unlike frozen/stunned). The only existing damage multiplier on the books was vanished's `damage_multiplier`, which lives on the ATTACKER and is consumed on first hit — vulnerable lives on the TARGET and persists for the status's full duration so the Arcanist's burst pattern (sunder → fireball → fireball) lands the amp across the whole window. Defensive: negative `duration` clamps to 0, negative `amp_pct` clamps to 0 (the resulting mod is exactly 1.0, never below — a hand-edited save can't park a `-50%` mod that would HEAL the target on hit). Default `+50%` matches the design note in the Run-47 audit's roadmap item #3.
- **`SHORT_CODES` + `DISPLAY_NAMES`** — `"vulnerable" → "VLN"` + `"vulnerable" → "Vulnerable"` so the above-the-sprite label renders `[VLN 2]` and the detail panel shows `Vulnerable · 2t · +50% taken`. New `summarize()` branch surfaces `+%d%% taken` for any status carrying a positive `damage_taken_pct` — vulnerable has no DPT and no armor mod, so without this segment the panel would read "Vulnerable · 2t" and tell the player nothing about WHY they should care. Reads the integer `damage_taken_pct` rather than the float `damage_taken_mod` so the line stays clean whatever rounding the multiplier would imply.
- **`stack()` collapser** — vulnerable doesn't sum or snowball. The existing `damage_per_turn` summer and `armor_mod` summer leave the new `damage_taken_mod` / `damage_taken_pct` fields untouched by default; this run adds a `damage_taken_mod = max(existing, new)` branch (and a mirroring `damage_taken_pct = max(...)` branch for the HUD). Two `+50%` applications stay at 1.5×, not compound to 2.25× — restacking refreshes the duration via the existing `duration` MAX collapser but never snowballs the multiplier. Protects against degenerate chains (sunder → sunder → sunder → fireball = 4×).
- **`src/combat/BattleEngine.gd`** — new TARGET-side vulnerable scan inside `_calculate_damage`, sitting alongside the existing attacker-side vanished scan. Loops the target's `status_effects` once, picks `max(1.0, max(damage_taken_mod over all vulnerable effects))`, multiplies the raw damage by it BEFORE the `max(1, raw)` floor at the return. MAX (not sum, not product) so two stacks behave consistently with the `stack()` HUD collapser — the player can't multiply the multiplier by stacking. The `if vuln_mod != 1.0` gate skips the multiplication when no vulnerable is present, so the no-status hot path is one extra dict scan and zero arithmetic.
- **`src/data/Abilities.gd`** — new `arcane_sunder` ability. Schema: `type: attack`, `target: single_enemy`, `range: 2` (Arcanist's standoff distance, matches arcane_surge), `base_damage: 12`, `max_charges: 2`, `cooldown_turns: 3`, `xp_cost: 50`, `icon_key: frost`, `applies_vulnerable: true`, `vuln_duration: 2`, `vuln_pct: 50`. Direct damage tuned modestly so the debuff is the headline — a 2-turn +50% window covers one follow-up fireball (which lands amplified) plus a same-turn AoE if the player has the charges. Cost mirrors eviscerate / concussive_slam tempo (2 charges / 3-turn / 50 xp) so the cross-class pool reads consistently.
- **`scenes/LevelUp.gd`** — `CLASS_UNLOCKS["arcanist"]` now reads `["arcane_sunder", "mana_shield", "backstab", "taunt"]`. Arcane Sunder FIRST so it's the class-unique pick the Arcanist sees at level 2 (mirrors the Run-46 idiom that put `eviscerate` first for rogue and the Run-47 idiom that put `concussive_slam` first for brawler). mana_shield stays as the secondary class-unique fallback, then the cross-class backstab/taunt options. The Arcanist now has TWO class-unique unlock options (sunder + shield) — covers both the offensive burst-setup and the defensive barrier idiom without forcing a single linear progression.
- **`scenes/BattleScene.gd`** — new `applies_vulnerable` branch in the on-hit status loop, sitting alongside the existing `applies_bleed` / `applies_stunned` / `applies_poisoned` branches. Reads `vuln_duration` + `vuln_pct` from the ability data, calls `StatusEffect.vulnerable(...)`, applies via `target.apply_status`, and refreshes the status label. Emits a combat-log line in `Color(0.78, 0.52, 1.0)` (soft purple — distinct from the red bleed, yellow stun, and green crit lines; matches the Run-44 WinScreen unlock banner accent so the arcane debuff reads as same-family) reading `Carl -> Goblin VULNERABLE +50% x2`. Speaks a System line via `speak_direct` carrying the pct + duration so the player sees both the visual feedback AND the auditory cue when the sunder lands. Effect-texture map gains `arcane_sunder -> fx_frost.png` (reuses the existing frost-nova violet ring — the arcane crack-the-armor moment reads as same-family; the VLN short-code above the target carries the "+50% taken" warning over the follow-up turn).
- **`autoloads/SystemVoice.gd`** — new `arcane_sunder` quip pool (6 lines, DCC-flavored: "Arcane Sunder. The target's defenses have been formally devalued.").
- **`tests/test_run48.gd`** (24 test functions, ~60 assertions): factory shape (default invocation locks the seven-field schema, default 2-turn +50% with mod=1.5, no DPT or armor mod), custom amp carries (+100% -> mod=2.0), negative-input clamps (negative duration and pct both go to 0; resulting mod is exactly 1.0 — can never reduce damage), zero-amp is a literal 1.0 multiplier; HUD helpers — short_code returns "VLN", display_name returns "Vulnerable", summarize includes "+50% taken" + carries duration, summarize omits "/turn" / "armor" / "skip turn" (no spurious DPT, armor mod, or skip-turn segments), custom amp formatting (+100% taken not +1% taken), stack collapses two vulnerables with MAX mod (1.5 vs 1.3 -> 1.5) and MAX pct, MAX direction-independent (weaker first, stronger second still picks stronger). Arcane Sunder schema — in `Abilities.DATA`, has all required fields, in `LevelUp.CLASS_UNLOCKS["arcanist"]` at index 0 with mana_shield/backstab retained as fallbacks, 2 charges / 3-turn cooldown / 50 xp tuning locked, direct damage ≤ 18 ("debuff is the headline" design lock), default amp = +50% and duration = 2t. BattleEngine — no-vulnerable raw lands in the expected variance band; +50% vulnerable strictly increases raw damage at same RNG seed; vulnerable NOT consumed on hit (status survives two _calculate_damage calls); two stacks don't compound (1.5× == one stack); +50% AND +100% stacked picks MAX (== +100% alone); minimum 1 raw damage floor holds even with 0% amp. End-to-end via Combatant: default 2-turn vulnerable expires after exactly two ticks with no HP damage; perform_attack against a sundered 0-armor target deals strictly more damage than against a plain target at same seed/no-crit; mana_shield untouched by vulnerable apply (regression guard against a defensive caller routing the debuff through take_damage). End-to-end Arcanist burst: hit1 + hit2 within the 2-turn window are amped (avg > plain hit3 after expiry). Cross-effect isolation: vulnerable doesn't carry skips_turn (regression guard) and doesn't register as frozen or stunned.
- **Test suite total: 2921 passed, 0 failed** (up from 2861 in Run 47; +60 new).

**Run 47 (Stun status effect + Concussive Slam ability — roadmap item #3 from the Run-46 audit; rounds the status palette to 8 effects and gives the Brawler a class-unique unlock distinct from the existing pushback strike):**
- **`src/combat/StatusEffect.gd`** — new `stunned(duration: int = 1) -> Dictionary` factory. Payload mirrors `frozen`'s `skips_turn: true` engine gate but WITHOUT the -2 armor mod — frozen is a ranged-spell control that locks the target down AND softens them up; stun is a melee impact that only steals tempo. Default 1-turn duration so a single Concussive Slam reads as "they lose one action," matching the player's expectation from the Brawler shield_bash idiom. Defensive: negative `duration` clamps to 0 so a hand-edited save can't park a -1-turn stun.
- **`SHORT_CODES` + `DISPLAY_NAMES`** — `"stunned" → "STN"` + `"stunned" → "Stunned"` so the above-the-sprite label renders `[STN 1]` and the detail panel shows `Stunned · 1t · skip turn`. New `summarize()` branch surfaces `skip turn` for any status carrying `skips_turn: true` (stun has no DPT/armor to display, so without this segment the panel would read "Stunned · 1t" and tell the player nothing). The same branch ALSO surfaces the skip cost for `frozen` (which has carried `skips_turn: true` since Run 1 but never advertised it on the panel) — a free UX win that doesn't break the existing Run-35 frozen-summarize test (it only asserts the presence of `Frozen`, `2t`, `-2 armor` and absence of `/turn`).
- **`src/combat/BattleEngine.gd`** — new `is_combatant_stunned(c)` helper parallels `is_combatant_frozen` (single-pass scan for `id == "stunned"`). The `enemy_ai_action` skip gate now reads `if is_combatant_frozen(enemy) or is_combatant_stunned(enemy)`, so a stunned enemy's AI turn is silently consumed exactly like a frozen one. Two helpers (rather than a generic `_skips_turn` reader) so HUD / quips / future cure-effects can address each effect distinctly.
- **`src/data/Abilities.gd`** — new `concussive_slam` ability. Schema: `type: attack`, `target: single_enemy`, `range: 1` (melee), `base_damage: 14`, `max_charges: 2`, `cooldown_turns: 3`, `xp_cost: 50`, `icon_key: axe`, `applies_stunned: true`, `stun_duration: 1`. Direct damage tuned modestly so the stun is the headline — a 1-turn skip on a tier-3 boss is worth more than the 14 hit on its own. Cost mirrors eviscerate's 2-charge / 3-turn / 50 xp tempo so the cross-class pool reads consistently.
- **`scenes/LevelUp.gd`** — `CLASS_UNLOCKS["brawler"]` now reads `["concussive_slam", "shield_bash"]`. Concussive Slam FIRST so it's the class-unique pick the Brawler sees at level 2 (mirrors the Run-46 idiom that put `eviscerate` first for rogue, and Run-21 for mana_shield on arcanist). shield_bash remains as the secondary fallback for players who already learned concussive_slam from a previous level. This is the FIRST time the Brawler's unlock list has had more than one entry — the lone shield_bash slot meant a Brawler past level 3 was stuck with stat upgrades only.
- **`scenes/BattleScene.gd`** — new `applies_stunned` branch in the on-hit status loop, sitting alongside the existing `applies_bleed` / `applies_poisoned` branches. Reads `stun_duration` from the ability data, calls `StatusEffect.stunned(...)`, applies via `target.apply_status`, and refreshes the status label. Emits a combat-log line in `Color(0.86, 0.72, 0.20)` (warm yellow — distinct from the red bleed and green crit lines) reading `Carl -> Goblin STUNNED x1`. Speaks a System line via `speak_direct` carrying the duration so the player sees both the visual feedback AND the auditory cue when the stun lands. Effect-texture map gains `concussive_slam -> fx_power_strike.png` (reuses the existing heavy-overhead VFX — stun needs no new visuals since the STN short-code above the target carries the visual weight on the following turn).
- **`autoloads/SystemVoice.gd`** — new `concussive_slam` quip pool (6 lines, DCC-flavored: "Concussive Slam. The enemy's turn has been administratively cancelled.").
- **`tests/test_run47.gd`** (18 test functions, ~40 assertions): factory shape (default invocation locks the six-field schema, no armor mod, skip flag present), custom-duration carries, negative-duration clamps to 0, zero-duration allowed; HUD helpers — short_code returns "STN", display_name returns "Stunned", summarize includes "Stunned · 1t · skip turn", summarize omits "/turn" and "armor" (no spurious DPT or armor mod), stack collapses two stuns with MAX duration (not summed — re-stunning doesn't chain into multi-turn lockdowns) and the skip flag survives the collapse. Concussive Slam schema — in `Abilities.DATA`, has all required fields, in `LevelUp.CLASS_UNLOCKS["brawler"]` at index 0 with shield_bash retained as fallback, 2 charges / 3-turn cooldown / 50 xp tuning locked, direct damage ≤ 18 ("stun is the headline" design lock). BattleEngine — `is_combatant_stunned` happy + negative paths, frozen-vs-stunned cross-fire guard (the two helpers must not register each other's effect). End-to-end via Combatant: 1-turn stun expires after exactly one tick with no HP damage, 2-turn stun survives the first tick and expires on the second, mana_shield untouched by stun apply (regression guard against a defensive caller routing stun through take_damage).
- **Test suite total: 2861 passed, 0 failed** (up from 2821 in Run 46; +40 new).
...
- Bleed status effect + Rogue's Eviscerate ability — new percent-of-max-HP
  ...
  Run 46
- **Status-effect depth — Vulnerable + Arcane Sunder.** The 9th status
  type and the first damage-AMP debuff. `StatusEffect.vulnerable(duration,
  amp_pct)` stores a `damage_taken_mod` float that the BattleEngine's
  `_calculate_damage` reads on the target side (mirroring the existing
  attacker-side vanished scan); applied at +50% for 2 turns by default.
  Doesn't deal DPT, doesn't change armor, doesn't lock a turn — it makes
  follow-up hits cost the target more, so it pairs naturally with the
  Arcanist's burst pattern (frost_nova lock → arcane_sunder apply →
  fireball lands amplified). Stack collapser takes MAX of any
  `damage_taken_mod` so restacking refreshes duration but never snowballs
  the multiplier; the engine scan likewise picks MAX so two stacks behave
  consistently. `arcane_sunder` (12 direct + vulnerable) heads the
  Arcanist's `CLASS_UNLOCKS` pool as their second class-unique unlock,
  ahead of mana_shield (the Arcanist now has two class-unique options —
  offensive setup vs. defensive barrier). HUD code `VLN` / display name
  `Vulnerable` / detail-panel suffix `+N% taken` — Run 48
- **Brawler sustain — Regenerating + Iron Resolve.** The 10th status type
  and the first POSITIVE per-turn ticker — every prior DoT (burning /
  poisoned / bleed) subtracts HP at tick time; regenerating adds HP via a
  new `heal_per_turn` field that `Combatant.tick_statuses` reads after the
  existing DoT drain (DoT first so a carrier on the brink can still die to
  burning — the regen tick can't paper over a lethal burn). `heal()` clamps
  to `max_hp - hp` so full-HP carriers waste the tick (no overheal pool);
  the `is_alive()` gate skips the heal if the carrier was killed by a same-
  tick DoT (no zombie revives). Stack collapser SUMS `heal_per_turn`
  (mirroring the DoT summer) so two regen buffs stack the per-tick heal
  for the longer of the two durations. `iron_resolve` (3-turn / 8 hpt = 24
  total HP) slots into the Brawler's `CLASS_UNLOCKS` between concussive_slam
  (Run 47 flagship) and shield_bash — the Brawler now has TWO class-unique
...
tests/
  ...
  test_run47.gd      — Stun status + Concussive Slam ability: StatusEffect.stunned factory (default shape, custom/negative/zero durations, skip flag present), HUD helpers (STN short code, Stunned display, summarize includes "skip turn", omits /turn and armor), stack collapses with MAX duration and skip flag survives, Concussive Slam schema (in DEFS, in brawler CLASS_UNLOCKS at index 0, tuning constants locked), engine helpers (is_combatant_stunned happy/negative + frozen/stunned cross-fire guard), end-to-end via Combatant (1-turn stun expires after one tick with no HP damage, 2-turn stun survives one tick and expires on second, mana_shield untouched by stun apply) (Run 47)
  test_run48.gd      — Vulnerable status + Arcane Sunder ability: StatusEffect.vulnerable factory (default 2-turn +50% with mod=1.5, custom amp scales linearly, negative-input clamps to 1.0 floor, zero-amp is no-op), HUD helpers (VLN short code, Vulnerable display, summarize includes "+N% taken", omits /turn/armor/skip turn, custom amp formatting), stack collapses with MAX mod + MAX pct (no compounding), Arcane Sunder schema (in DEFS, in arcanist CLASS_UNLOCKS at index 0 with mana_shield/backstab retained, tuning constants locked, default amp+duration), BattleEngine integration (no-vulnerable variance band, +50% strictly amps raw damage at same seed, not consumed on hit, two stacks don't compound, MAX picks stronger amp, min-1 raw floor holds at 0% amp), end-to-end via Combatant (default 2-turn expires after two ticks with no HP damage, perform_attack deals strictly more to sundered target at no-crit, mana_shield untouched by vulnerable apply, Arcanist burst — two amped hits avg > plain hit after expiry), cross-effect isolation (no skips_turn, doesn't register as frozen/stunned) (Run 48)

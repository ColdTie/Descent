...
func generate(p_floor: int, rng: RandomNumberGenerator) -> void:
	...
	# Compute the hero's adjacent hexes (must remain passable floor)
	...
	for i: int in range(lava_count):
		...
		# Never lava on center or any of the 6 adjacent hexes — hero must be able to move
		if h == Vector2i.ZERO or h in hero_neighbors:
			continue
		tile_types[h] = "lava"
		passable[h] = false
